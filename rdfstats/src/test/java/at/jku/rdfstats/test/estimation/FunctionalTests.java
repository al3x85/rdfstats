...
public class FunctionalTests extends TestCase {
	...
	public void testJoin() throws RDFStatsModelException {
		...
		Integer[] join = ds.triplesForQuery(Constants.BSBM_PREFIXES + "SELECT * WHERE { { producer1:Product1 bsbm:productFeature ?f } { ?f a bsbm:ProductFeature ; rdfs:label ?l } }");	
		System.out.println("join: " + join[0] + " - " + join[1] + " - " + join[2]);
		Integer[] crossProd = ds.triplesForQuery("SELECT * WHERE { { ?s ?p ?o } { ?a ?b ?c } }");
		System.out.println("cross: " + crossProd[0] + " - " + crossProd[1] + " - " + crossProd[2]);
		Integer[] numFeature = ds.triplesForQuery(Constants.BSBM_PREFIXES + "SELECT * WHERE { ?s a bsbm:Product ; bsbm:productPropertyNumeric1 ?n . FILTER (?n > 100) }");
		System.out.println("numeric feature: " + numFeature[0] + " - " + numFeature[1] + " - " + numFeature[2]);
		Integer[] labelOrder = ds.triplesForQuery(Constants.BSBM_PREFIXES + "SELECT * WHERE { ?s a bsbm:Product ; rdfs:label ?l . FILTER (?l > 'z') }");
